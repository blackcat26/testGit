				...
				<div class="col-xs-12" id="questionbar" style="display:none;">
					<dl class="clear" id="questionindex">
						...
						<dd class="tableindex">
			                ...
			                {x2;eval: v:tid++}
			                {x2;eval: v:qmid++}
							<a style="margin-bottom:0.5rem;" id="sign_{x2;v:question['questionid']}" href="javascript:;" onclick="javascript:$.leftMenu($('#questionbar'));gotoquestion('{x2;v:question['questionid']}');$('#questionbar').queue(function(){$('#form1').toggle();$(this).dequeue();});" class="btn btn-default{x2;if:$sessionvars['examsessionsign'][v:question['questionid']]} btn-danger{x2;endif}">{x2;v:tid}</a>
							{x2;endtree}
							{x2;tree:$sessionvars['examsessionquestion']['questionrows'][v:quest],questionrow,qrid}
			                {x2;eval: v:tid++}
			                {x2;tree:v:questionrow['data'],data,did}
			                {x2;eval: v:qmid++}
			                <a style="margin-bottom:0.5rem;" id="sign_{x2;v:data['questionid']}" href="javascript:;" onclick="javascript:$.leftMenu($('#questionbar'));gotoquestion('{x2;v:data['questionid']}');$('#questionbar').queue(function(){$('#form1').toggle();$(this).dequeue();});" class="btn btn-default{x2;if:$sessionvars['examsessionsign'][v:data['questionid']]} btn-danger{x2;endif}">{x2;v:tid}-{x2;v:did}</a>
	            			{x2;endtree}
	            			{x2;endtree}
						</dd>
						{x2;endif}
					...
					<div class="paperexamcontent" id="questions_{x2;v:question['questionid']}" rel="{x2;v:quest}" style="clear:both;overflow:hidden;background:#FFFFFF;margin-top:0.5rem;padding:1rem;">
						<h4 class="title">
							...
							<span class="pull-right">
								<a class="hidden btn {x2;if:$sessionvars['examsessionsign'][v:question['questionid']]}text-danger{x2;else}text-info{x2;endif} qicon" href="javascript:;" onclick="javascript:signQuestion('{x2;v:question['questionid']}',this);"><i class="glyphicon glyphicon-bookmark"></i></a>
						...
						<div class="selector text-nowrap selectorNav" >
	                    	{x2;if:$questype[v:quest]['questchoice'] == 1 || $questype[v:quest]['questchoice'] == 4}
		                        ...
		                        {x2;eval: break;}
		                        {x2;endif}
		                        <label class="radio-inline img-circle text-center selectorItem"><input type="radio" class="hidden" name="question[{x2;v:question['questionid']}]" rel="{x2;v:question['questionid']}" value="{x2;v:so}" {x2;if:v:so == $sessionvars['examsessionuseranswer'][v:question['questionid']]}checked{x2;endif}/>
		                            {x2;if:$questype[v:quest]['questchoice'] == 4}
		                                {x2;if:v:so=='A'}对
		                                {x2;else}错
		                                {x2;endif}
		                            {x2;else}
		                        ...
		                        {x2;if:v:qcid < v:qmid}
                                <a class="pull-right btn btn-default selectorNext" onclick="javascript:gotoindexquestion({x2;v:qcid});">下一题<span class="glyphicon glyphicon-chevron-right"></span></a>
                                {x2;endif}
	                        ...
	                        {x2;else}
		                        ...
		                        {x2;eval: break;}
		                        {x2;endif}
		                        <label class="checkbox-inline" style="line-height:3.4rem;"><input type="checkbox" name="question[{x2;v:question['questionid']}][{x2;v:key}]" rel="{x2;v:question['questionid']}" value="{x2;v:so}" {x2;if:in_array(v:so,$sessionvars['examsessionuseranswer'][v:question['questionid']])}checked{x2;endif}/>{x2;v:so}</label>
                                {x2;endtree}
