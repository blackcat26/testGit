...
<?php } ?>
		...
		<div class="container-fluid" style="padding-bottom:4.7rem;padding-top:5.6rem;">
			<div class="row-fluid">
			    <!--考题跳转导航-->
 ...
 $qnid++; ?>
			                <?php $tid++; ?>
			                <?php $qmid++; ?>
							<a style="margin-bottom:0.5rem;" id="sign_<?php echo $question['questionid']; ?>" href="javascript:;" onclick="javascript:$.leftMenu($('#questionbar'));gotoquestion('<?php echo $question['questionid']; ?>');$('#questionbar').queue(function(){$('#form1').toggle();$(this).dequeue();});" class="btn btn-default<?php if($this->tpl_var['sessionvars']['examsessionsign'][$question['questionid']]){ ?> btn-danger<?php } ?>"><?php echo $tid; ?></a>
							<?php } ?>
							<?php $qrid = 0;
 ...
 $did++; ?>
			                <?php $qmid++; ?>
			                <a style="margin-bottom:0.5rem;" id="sign_<?php echo $data['questionid']; ?>" href="javascript:;" onclick="javascript:$.leftMenu($('#questionbar'));gotoquestion('<?php echo $data['questionid']; ?>');$('#questionbar').queue(function(){$('#form1').toggle();$(this).dequeue();});" class="btn btn-default<?php if($this->tpl_var['sessionvars']['examsessionsign'][$data['questionid']]){ ?> btn-danger<?php } ?>"><?php echo $tid; ?>-<?php echo $did; ?></a>
	            			<?php } ?>
	            			<?php } ?>
						</dd>
						<?php } ?>
					...
					<div class="paperexamcontent" id="questions_<?php echo $question['questionid']; ?>" rel="<?php echo $quest; ?>" style="clear:both;overflow:hidden;background:#FFFFFF;margin-top:0.5rem;padding:1rem;">
						<h4 class="title">
							...
							<span class="pull-right">
								<a class="hidden btn <?php if($this->tpl_var['sessionvars']['examsessionsign'][$question['questionid']]){ ?>text-danger<?php } else { ?>text-info<?php } ?> qicon" href="javascript:;" onclick="javascript:signQuestion('<?php echo $question['questionid']; ?>',this);"><i class="glyphicon glyphicon-bookmark"></i></a>
	                    ...
	                    <?php } ?>
						<div class="selector text-nowrap selectorNav" >
 ...
 $sid++; ?>
		                        ...
		                        <?php break;; ?>
		                        <?php } ?>
		                        <label class="radio-inline img-circle text-center selectorItem"><input type="radio" class="hidden" name="question[<?php echo $question['questionid']; ?>]" rel="<?php echo $question['questionid']; ?>" value="<?php echo $so; ?>" <?php if($so == $this->tpl_var['sessionvars']['examsessionuseranswer'][$question['questionid']]){ ?>checked<?php } ?>/>
		                            <?php if($this->tpl_var['questype'][$quest]['questchoice'] == 4){ ?>
		                                <?php if($so=='A'){ ?>对
		                                <?php } else { ?>错
		                                <?php } ?>
		                            <?php } else { ?>
		                        ...
		                        <?php if($qcid < $qmid){ ?>
                                <a class="pull-right btn btn-default selectorNext" onclick="javascript:gotoindexquestion(<?php echo $qcid; ?>);">下一题<span class="glyphicon glyphicon-chevron-right"></span></a>
                                <?php } ?>
 ...
 $sid++; ?>
		                        ...
		                        <?php break;; ?>
		                        <?php } ?>
		                        <label class="checkbox-inline" style="line-height:3.4rem;"><input type="checkbox" name="question[<?php echo $question['questionid']; ?>][<?php echo $key; ?>]" rel="<?php echo $question['questionid']; ?>" value="<?php echo $so; ?>" <?php if(in_array($so,$this->tpl_var['sessionvars']['examsessionuseranswer'][$question['questionid']])){ ?>checked<?php } ?>/><?php echo $so; ?></label>
                                <?php } ?>
 ...
 $sid++; ?>
		...
		<div class="container-fluid text-center" style="background-color:#337AB7;z-index:9999;position:fixed;top:0px;border-top:1px solid #E9E9E9;padding:0.8rem 15px;">
			<div style="clear:both;background-color:#337AB7;padding-top:0px;border:0px;padding:0rem;padding-bottom:0.8rem;" class="input-group-addon footer">
				<div class="col-xs-3">
					<div class="text-center" style="height:3.2rem;padding:0.2rem;">
						<a href="javascript:;" onclick="gotopreface();">
							<h5 style="line-height:2rem;color:#FFFFFF;"><em class="glyphicon glyphicon-th"></em> 题序</h5>
						</a>
					</div>
				</div>
				<div class="col-xs-6">
					<div class="text-center hidden" style="height:3.2rem;padding:0.2rem;>
						<a href="index.php?item-phone-cart" class="ajax" data-page="cart" data-target="cart">
							<h4 style="line-height:2rem;color:#FFFFFF;"><em class="glyphicon glyphicon-time"></em> <span id="timer_h">00</span><span>：</span><span id="timer_m">00</span><span>：</span><span id="timer_s">00</span></h4>
						</a>
					</div>
				</div>
				<div class="col-xs-3">
					<div class="text-center" style="height:3.2rem;padding:0.2rem;">
						<a href="#submodal" role="button" data-toggle="modal">
							<h5 style="line-height:2rem;color:#FFFFFF;"><em class="glyphicon glyphicon-list-alt"></em> 交卷</h5>
						</a>
					</div>
				</div>
			</div>
		</div>
		<div class="modal fade" id="submodal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="margin-top:5.6rem;">
		...
		$(document).ready(function(){
			...
			$('#form1 :input[type=radio]').change(function(){
				...
				var _this=$(this);
				var p=[];
				var o=_this.parent().siblings('.bg-primary');
				p.push(_.name);
				if(_.checked){
				    if(!!o){
				        o.removeClass('bg-primary');
				    }
				    _this.parent().addClass('bg-primary');
					p.push(_.value);
					p.push(Date.parse(new Date())/1000);
					$('#time_'+_this.attr('rel')).val(Date.parse(new Date())/1000);
					set.apply(formData,p);
